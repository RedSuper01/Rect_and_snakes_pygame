...
victory_fon_dict_coord = {'1': ['victory_fon1.jfif'], '2': ['victory_fon2.jfif'], '3': ['victory_fon3.jfif'],
                          '4': ['victory_fon4.jfif'], '5': ['victory_fon5.jfif'], '6': ['victory_fon6.jfif'],
                          '7': ['victory_fon7.jfif'],
                          'victory': 'victory'}
losing_fon_dict_coord = {'1': ['losing_fon1.jfif'], '2': ['losing_fon2.jfif'], 'losing': 'losing'}
...
def victory_screen(number_of_level):
    ...
    try_again_sprite.image = pygame.transform.scale(load_image('try_again2.png', -1), (200, 200))
    ...
    exit_button_sprite.image = pygame.transform.scale(load_image('exit_button_5.jfif', -1), (200, 200))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                x, y = event.pos
                if 350 <= x <= 550 and 562 <= y <= 762:
                    count_of_done_cuts = 0
                    launch_level(number_of_level, x1, y1, x2, y2)
                    break
                elif 500 <= x <= 700 and 562 <= y <= 762:
                    count_of_done_cuts = 0
                    splash_screen()
                    break
...
def launch_level(number_of_level, x1, y1, x2, y2):
    ...
    cursor.image = pygame.transform.scale(load_image('cursor5.png', -1), (30, 30))
    cursor.rect = cursor.image.get_rect()
    cursor.rect.x, cursor.rect.y = x1 - 15, y1 - 15
    all_sprites.add(cursor)

    try_again_sprite = pygame.sprite.Sprite()
    try_again_sprite.image = load_image('try_again2.png', -1)
    try_again_sprite.rect = try_again_sprite.image.get_rect()
    ...
    exit_sprite.image = load_image('exit_button_5.jfif', -1)
    ...
    for i in range(n):
        if snake_fon == 'snake1.png':
            ...
            bg = AnimatedSprite(load_image('snake_animation2.png'), 5, 3, 130, 130, x1 + 50, y1 + 50)
        all_snakes.add(bg)
        dc_snakes[i] = bg
            ...
            if pause:
                ...
                if lines_type.check_game():
                    ...
                    if count_of_done_cuts < count_cuts:
                        ...
                        connection = sqlite3.connect('pygame_rect_result_game.db')
...
def changing_design():
    ...
    accept_button_sprite.image = pygame.transform.scale(load_image('accept_button3.jpg', -1), (300, 300))
